...
fn parse_instruction(ts: &mut TokenStream) -> ParseResult<Instruction> {
    ...
    match ts.peek() {
        Some(Token::GrowStack) => {
            ts.advance();
            Ok(Instruction::GrowStack(ts.expect_int()?))
        }
        Some(Token::ShrinkStack) => {
            ts.advance();
            Ok(Instruction::ShrinkStack(ts.expect_int()?))
        }
        Some(Token::LoadArg) => {
            ts.advance();
            Ok(Instruction::LoadArg(ts.expect_int()?))
        }
        Some(Token::StoreArg) => {
            ts.advance();
            Ok(Instruction::StoreArg(ts.expect_int()?))
        }
        Some(Token::ArgAddi) => {
            ts.advance();
            Ok(Instruction::ArgAddi(ts.expect_int()?))
        }
        Some(Token::ArgSubi) => {
            ts.advance();
            Ok(Instruction::ArgSubi(ts.expect_int()?))
        }
        Some(Token::Add) => {
            ts.advance();
            Ok(Instruction::Add)
        ...
        Some(Token::Push) => {
            ts.advance();
            Ok(Instruction::Push(ts.expect_int()?))
        }
        Some(Token::PushImm) => {
            ts.advance();
        ...
        Some(Token::PushResult) => {
            ts.advance();
            Ok(Instruction::PushResult)
        }
        Some(Token::Call) => {
            ts.advance();
            Ok(Instruction::Call(ts.expect_ident()?))
        }
        Some(Token::Jmp) => {
            ts.advance();
            Ok(Instruction::Jmp(ts.expect_ident()?))
        }
        Some(Token::Jnz) => {
            ts.advance();
            Ok(Instruction::Jnz(ts.expect_ident()?))
        }
        Some(Token::JnzPause) => {
            ts.advance();
            Ok(Instruction::JnzPause(ts.expect_ident()?))
        }
        Some(Token::JnzSet) => {
            ts.advance();
            Ok(Instruction::JnzSet(ts.expect_ident()?))
        }
        Some(Token::JzSet) => {
            ts.advance();
            Ok(Instruction::JzSet(ts.expect_ident()?))
        }
        Some(Token::JzPause) => {
            ts.advance();
            Ok(Instruction::JzPause(ts.expect_ident()?))
        }
        Some(Token::Jz) => {
            ts.advance();
            Ok(Instruction::Jz(ts.expect_ident()?))
        }
        Some(Token::Jeq) => {
            ts.advance();
            Ok(Instruction::Jeq(ts.expect_ident()?))
        }
        Some(Token::JeqImm) => {
            ts.advance();
            ts.advance();
        ...
        Some(Token::Lea) => {
            ts.advance();
            Ok(Instruction::Lea(ts.expect_ident()?))
        }
        Some(Token::Retv) => {
            ts.advance();
        ...
        Some(Token::Ret) => {
            ts.advance();
            Ok(Instruction::Ret(ts.expect_int()?))
        }
        Some(Token::LStr) => {
            ts.advance();
        ...
        Some(Token::DelayNeq0) => {
            ts.advance();
            Ok(Instruction::DelayNeq0)
        }
        Some(Token::Exit1) => {
            ts.advance();
            Ok(Instruction::Exit1)
        }
        Some(Token::Exit2) => {
            ts.advance();
