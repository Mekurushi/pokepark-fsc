...
fn parse_instruction(ts: &mut TokenStream) -> ParseResult<Instruction> {
    ...
    match ts.peek() {
        ...
        Some(Token::Mul)       => { ts.advance(); Ok(Instruction::Mul) }
        Some(Token::Div)       => { ts.advance(); Ok(Instruction::Div) }
        ...
        Some(Token::Not)       => { ts.advance(); Ok(Instruction::Not) }
        ...
        Some(Token::Fdiv)       => { ts.advance(); Ok(Instruction::Fdiv) }
        Some(Token::Feq0)       => { ts.advance(); Ok(Instruction::Feq0) }
        Some(Token::Fneg)       => { ts.advance(); Ok(Instruction::Fneg) }
        Some(Token::Feq)       => { ts.advance(); Ok(Instruction::Feq) }
        Some(Token::Fneq)       => { ts.advance(); Ok(Instruction::Fneq) }
        Some(Token::Flt)       => { ts.advance(); Ok(Instruction::Flt) }
        ...
        Some(Token::Neq) => {ts.advance(); Ok(Instruction::Neq)}
        Some(Token::Lt) => {ts.advance(); Ok(Instruction::Lt)}
        Some(Token::Gt) => {ts.advance(); Ok(Instruction::Gt)}
        Some(Token::Le) => {ts.advance(); Ok(Instruction::Le)}
        Some(Token::Ge) => {ts.advance(); Ok(Instruction::Ge)}
